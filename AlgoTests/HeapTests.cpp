...
namespace heapTests
{
    ...
    {
        const ansa::Heap<int> heap({3, 2, 1, 7, 8, 4, 10, 16, 12});
    ...
    {
        // Act
        auto object = ansa::Heap<int>({4, 1, 3, 2, 16, 9, 10, 14, 8, 7});
        // Arrange
        object.buildHeap();
        const auto& heap           = object.getHeap();
        const auto  equalityObject = std::vector<int>({16, 14, 10, 8, 7, 9, 3, 2, 4, 1});
        // Assert
